...
// - If `window.__API_BASE__` is set, use it (useful for quick overrides).
// - If the frontend is served from a static dev server (e.g. Live Server on port 5500),
//   assume backend runs on `http://localhost:3000/api`.
// - Otherwise use a relative `/api` so proxies or the same origin work in production.
declare global {
  interface Window {
    __API_BASE__?: string;
  }
 ...
 */
export async function fetchProfiles(queryParams: Record<string, any> = {}): Promise<any> {
 ...
 */
export async function fetchProfileById(id: number): Promise<object> {
 ...
 * @returns {Promise<object>} The profile object.
 */
export async function fetchProfileByUserId(userId: number): Promise<object> {
 ...
 */
export async function updateProfile(id: number, updateProfileDto: object): Promise<object> {
  const response = await fetch(`${PROFILES_URL}/${id}`, {
    method: 'PATCH',
    headers: {
 ...
 * @returns {Promise<object>} The updated profile object.
 */
export async function updateAvatar(id: number, formData: FormData): Promise<object> {
 ...
 */
export async function deleteProfile(id: number): Promise<object> {
  const response = await fetch(`${PROFILES_URL}/${id}`, {
    method: 'DELETE',
  });
  return handleResponse(response);
